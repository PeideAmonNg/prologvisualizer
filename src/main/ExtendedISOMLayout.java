 ...
 */
public class ExtendedISOMLayout<V, E> extends AbstractLayout<V,E> implements IterativeContext {
	...
	public ExtendedISOMLayout(Graph<V,E> g, Stack<Node> mainBranch) {
		super(g);
	}
...
//			done = true;
	...
	private synchronized void adjustVertex(V v, Point2D tempXYD) {
		...
		while (!queue.isEmpty()) {
			...
			Point2D currXYData = new Point((int)super.getX(current), (int)super.getY(current));
